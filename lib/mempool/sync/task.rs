...
impl std::fmt::Display for ApplySyncActionTimeoutError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let Some(action) = action else {
            return write!(
                f,
                "Timeout after {timeout:?} while waiting to apply sync action"
            );
        };
        match action {
            SyncAction::SequenceMessage(SequenceMessage::BlockHash(
                block_hash_msg,
            )) => {
            ...
            SyncAction::InsertTx(txid) => {
                write!(
                    f,
                    "Timeout after {timeout:?} while waiting to apply insert tx ({txid}) action",
                )
            }
        }
    }
    ...
    BorrowedEnforcer: CusfEnforcer,
{
    ...
    for tx_info in resp_block.tx.iter().rev() {
        ...
        {
            sync_state.tx_cache.insert(restored_txid, restored_tx);
            sync_state.action_queue.push_front(InsertTx(restored_txid));
        }
    }
    match sync_state
        .action_queue
