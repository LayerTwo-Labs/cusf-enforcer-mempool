...
use bitcoin::{
    Amount, BlockHash, OutPoint, Transaction, TxIn, Txid, Weight,
...
use crate::{
    ...
    mempool::{
        ...
        sync::{
            ApplySyncActionResult, ApplySyncActionTimeoutError, RequestItem,
            SyncAction, SyncActionQueue, SyncActionQueueHead,
        },
    },
    zmq::{
    ...
    for tx_info in &block.tx {
            ...
            .map_err(cusf_enforcer::Error::AcceptTx)?
        {
            cusf_enforcer::TxAcceptAction::Accept {
                ...
                let modified_weight_wu = decoded
                    .weight()
                    .to_wu()
                    .saturating_add_signed(weight_tweak);
    ...
    Enforcer: CusfEnforcer,
{
    ...
    let Some(SyncActionQueueHead {
        action:
            SyncAction::SequenceMessage(SequenceMessage::BlockHash(block_hash_msg)),
        reached_head_time: _,
    ...
    Enforcer: CusfEnforcer,
{
    match seq_msg {
        ...
        SequenceMessage::TxHash(TxHashMessage {
            ...
            let added = try_add_tx_from_cache(inner, sync_state, txid).await?;
            if !added {
                tracing::debug!(
                    %txid,
        ...
        SequenceMessage::TxHash(TxHashMessage {
            ...
            if !removed {
                tracing::debug!(
                    %txid,
                    "skipped removal, tx already not in mempool",
                );
            }
        ...
        match msg {
            ...
            CombinedStreamItem::ApplySyncActionTimeout => {
                let err = ApplySyncActionTimeoutError {
                    action: sync_state.action_queue.pop_front(),
                };
                return Err(err.into());
            }
