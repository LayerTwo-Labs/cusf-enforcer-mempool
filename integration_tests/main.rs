...
use cusf_enforcer_mempool_integration_tests::{
    ...
    test_disconnect_below_rejected_block, test_disconnect_through_sync_tip,
...
fn run() -> anyhow::Result<std::process::ExitCode> {
    ...
    let setup_tests: &[(&str, SetupFn)] = &[
        ...
        ("disconnect_below_rejected_block", |s| {
            Box::pin(
                test_disconnect_below_rejected_block::test_disconnect_below_rejected_block(s),
            )
        }),
